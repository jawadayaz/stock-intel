...
async function ensureCriteriaSeeded(env) {
  ...
  if (!existing) {
    const defaults = await fetch(new URL("./criteria_defaults.json", import.meta.url)).then(r => r.json()).catch(() => null);
    if (defaults) await kvPut(env, "criteria_config", defaults);
  }
}
